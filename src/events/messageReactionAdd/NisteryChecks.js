...
async function nisteryPlayerJoined(client, user, message) {
...
async function nisteryLynchVoting(client, reaction, user) {
...
module.exports = async function nisteryChecks(client, reaction, user) {
  if (!client.nistery) return;  // game is not active

  const emoji = reaction.emoji.name;

  if (reaction.message.id === client.nistery.joiningMessageID && emoji === '🔪')
    await nisteryPlayerJoined(client, user, reaction.message);
  if (reaction.message.id === client.nistery.voteMessage?.id)
    await nisteryLynchVoting(client, reaction, user);
}
